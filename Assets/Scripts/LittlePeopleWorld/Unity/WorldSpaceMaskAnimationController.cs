...
namespace LittlePeopleWorld.Unity
{
    public sealed class WorldSpaceMaskAnimationController : MonoBehaviour
    {
        ...
        // 羽は「左右ペア」から「背中側の遠近ペア(Far/Near)」に意味を変更した。
        // ・BackOffsetRatio: 背中側(-x方向)へのオフセット
        // ・UpOffsetRatio:   上方向へのオフセット(Farのほうを高めに置く)
        // ・BaseAngleDegrees: 静止時の羽の角度(背中から斜め上に伸びる基準姿勢)
        [Header("Particle Fairy Wings")]
        [SerializeField] bool showParticleWings = true;
        [SerializeField] float particleWingFarBackOffsetRatio = 0.30f;
        [SerializeField] float particleWingFarUpOffsetRatio = 0.34f;
        [SerializeField] float particleWingFarBaseAngleDegrees = 52f;
        [SerializeField] float particleWingNearBackOffsetRatio = 0.22f;
        [SerializeField] float particleWingNearUpOffsetRatio = 0.18f;
        [SerializeField] float particleWingNearBaseAngleDegrees = 34f;
        ...
        [SerializeField] float particleWingFlapAngleDegrees = 12f;

        // Step P-0/P-2: 直立化(頭・脚)と向き反転・傾きのための追加パラメータ。
        [Header("Particle Fairy Body")]
        [SerializeField] bool showParticleLegs = true;
        [SerializeField] float particleHeadOffsetYRatio = 0.62f;
        [SerializeField] float particleLegLengthRatio = 0.48f;
        [SerializeField] float particleLegWidthRatio = 0.22f;
        [SerializeField] float particleLegSwingDegrees = 14f;
        [SerializeField] float particleLegClimbSwingDegrees = 22f;
        [SerializeField] float particleLegBurstSwingDegrees = 40f;
        [SerializeField] float particleFlipThresholdPxPerSec = 8f;
        [SerializeField] float particleTiltClampDegrees = 28f;
        [SerializeField] float particleTiltBurstClampDegrees = 80f;
        [SerializeField] float particleTiltClimbClampDegrees = 12f;
        [SerializeField] float particleTiltDegPerPxSpeed = 0.35f;
        [SerializeField] float particleTiltSmoothLerp = 10f;
        [SerializeField] float particleHoverBobSpeed = 2f;
        [SerializeField] float particleHoverBobRatio = 0.05f;
        ...
        Particle CreateParticle(int index)
        {
            ...
            // Glowはroot直下(向き反転・傾きの影響を受けない)。
            var glow = CreateRenderer(root, "Glow", RuntimeSpriteFactory.Circle, -4);

            // Figureは向き反転(localScale.x = ±1)と傾き(localRotation)だけを担当する中間ノード。
            // +x=向いている方向、+y=上、という約束でこの下の子要素はすべて配置する。
            var figureObject = new GameObject("Figure");
            figureObject.transform.SetParent(root, false);
            var figure = figureObject.transform;

            // 羽は「左右ペア」ではなく「背中側の遠近ペア」として、Figure配下・体より背中寄りに置く。
            var farWing = CreateRenderer(figure, "Wing Far", RuntimeSpriteFactory.Circle, 9);
            var nearWing = CreateRenderer(figure, "Wing Near", RuntimeSpriteFactory.Circle, 9);
            var body = CreateRenderer(figure, "Body", RuntimeSpriteFactory.Circle, 10);
            var head = CreateRenderer(figure, "Head", RuntimeSpriteFactory.Circle, 11);
            var legBack = CreateRenderer(figure, "Leg Back", RuntimeSpriteFactory.Capsule, 9);
            var legFront = CreateRenderer(figure, "Leg Front", RuntimeSpriteFactory.Capsule, 10);

            // 頭は真上(固定の斜めオフセットを廃止し、particleSize比率で統一)。
            head.transform.localPosition = new Vector3(0f, particleSize * particleHeadOffsetYRatio, 0f);

            // 脚の付け根位置(Figureローカル、右向き基準)。反転はFigure側で行うため左右対称の値でよい。
            legBack.transform.localPosition = new Vector3(-particleSize * 0.10f, -particleSize * 0.42f, 0f);
            legFront.transform.localPosition = new Vector3(particleSize * 0.10f, -particleSize * 0.42f, 0f);
            ...
            body.transform.localScale = new Vector3(particleSize * 0.82f, particleSize * 1.05f, 1f);
            head.transform.localScale = Vector3.one * particleSize * 0.58f;
            var legScale = new Vector3(
                particleSize * particleLegWidthRatio,
                particleSize * particleLegLengthRatio,
                1f);
            legBack.transform.localScale = legScale;
            legFront.transform.localScale = legScale;

            body.color = color;
            head.color = Color.Lerp(color, Color.white, 0.35f);
            var legColor = Color.Lerp(color, Color.white, 0.2f);
            legBack.color = legColor;
            legFront.color = legColor;

            var wingColor = Color.Lerp(color, Color.white, 0.7f);
            wingColor.a = particleWingAlpha;
            farWing.color = wingColor;
            nearWing.color = wingColor;
            ...
            return new Particle
            {
                ...
                FigureRoot = figure,
                GlowRenderer = glow,
                FarWingRenderer = farWing,
                NearWingRenderer = nearWing,
                BodyRenderer = body,
                HeadRenderer = head,
                LegBackRenderer = legBack,
                LegFrontRenderer = legFront,
                Facing = 1f
            };
        }
        ...
        void RenderParticle(Particle particle, int index)
        {
            ...
            // rootは位置専用。Viewは使い回されるため、前フレームの回転を残さないよう毎フレーム明示する。
            particle.Root.rotation = Quaternion.identity;

            var pose = SelectParticlePose(particle);

            UpdateParticleFacing(particle, pose);
            UpdateParticleTilt(particle, pose);

            particle.FigureRoot.localScale = new Vector3(particle.Facing, 1f, 1f);
            particle.FigureRoot.localRotation = Quaternion.Euler(0f, 0f, particle.Tilt);

            // ホバーの上下ゆれ(通常飛行〜吸着状態まで共通の“生きている”感)。
            var hover = Mathf.Sin(Time.time * particleHoverBobSpeed + index * 0.6f) * particleSize * particleHoverBobRatio;
            particle.FigureRoot.localPosition = new Vector3(0f, hover, 0f);
            ...
            RenderParticleWings(particle, index, pose);
            RenderParticleLegs(particle, index, pose);
        }

        // Particleの見た目状態(ポーズ)。物理側のフラグ(IsClimbing/IsBloomAttached/BurstFreeTimer)
        // をそのまま描画パラメータへ写像する。優先順位はUpdateParticleの分岐と揃えている:
        // Burst > BloomAttached > Climbing > 通常飛行。
        enum ParticlePoseKind
        {
            Flying,
            Climbing,
            BloomAttached,
        ...
        readonly struct ParticlePose
        {
            public readonly ParticlePoseKind Kind;
            public ParticlePose(ParticlePoseKind kind) => Kind = kind;
        }

        static ParticlePose SelectParticlePose(Particle particle)
        {
            if (particle.BurstFreeTimer > 0f)
            {
                return new ParticlePose(ParticlePoseKind.Bursting);
            }

            if (particle.IsBloomAttached)
            {
                return new ParticlePose(ParticlePoseKind.BloomAttached);
            }

            if (particle.IsClimbing)
            {
                return new ParticlePose(ParticlePoseKind.Climbing);
            }

            return new ParticlePose(ParticlePoseKind.Flying);
        }

        // 速度のx成分にヒステリシスを設けて向きを更新する。
        // 閾値なしだと、操舵やジッターによる微小な速度反転で毎フレーム左右がパタつく。
        void UpdateParticleFacing(Particle particle, ParticlePose pose)
        {
            if (pose.Kind == ParticlePoseKind.BloomAttached)
            {
                // 花に吸着中は向きを変えない(その場で静止しているため)。
                return;
            }

            if (Mathf.Abs(particle.Vel.x) > particleFlipThresholdPxPerSec)
            {
                particle.Facing = particle.Vel.x >= 0f ? 1f : -1f;
            }
        }

        // 縦速度に比例した傾き(Atan2は使わない: 真上/真下付近で角度が不連続に暴れるため)。
        // 生の値だと操舵・分離力・ジッターで震えるため、LerpAngleで平滑化してから適用する。
        void UpdateParticleTilt(Particle particle, ParticlePose pose)
        {
            var clamp = pose.Kind switch
            {
                ParticlePoseKind.Bursting => particleTiltBurstClampDegrees,
                ParticlePoseKind.Climbing => particleTiltClimbClampDegrees,
                ParticlePoseKind.BloomAttached => 0f,
                _ => particleTiltClampDegrees
            };

            var targetTilt = Mathf.Clamp(particle.Vel.y * particleTiltDegPerPxSpeed, -clamp, clamp);
            var lerpFactor = 1f - Mathf.Exp(-particleTiltSmoothLerp * Time.deltaTime);
            particle.Tilt = Mathf.LerpAngle(particle.Tilt, targetTilt, lerpFactor);
        }

        void RenderParticleWings(Particle particle, int index, ParticlePose pose)
        {
            if (particle.FarWingRenderer == null || particle.NearWingRenderer == null)
            {
                return;
            }

            particle.FarWingRenderer.enabled = showParticleWings;
            particle.NearWingRenderer.enabled = showParticleWings;
            if (!showParticleWings)
            {
                return;
            }

            // ポーズごとの羽ばたき速度・振幅倍率(たたむ/全開にするの表現)。
            float flapSpeedScale;
            float flapLengthScale;
            switch (pose.Kind)
            {
                case ParticlePoseKind.Climbing:
                    flapSpeedScale = 0.4f;
                    flapLengthScale = 0.7f;
                    break;
                case ParticlePoseKind.BloomAttached:
                    flapSpeedScale = 0.5f;
                    flapLengthScale = 1.3f;
                    break;
                case ParticlePoseKind.Bursting:
                    flapSpeedScale = 1f;
                    flapLengthScale = 1.3f;
                    break;
                default:
            ...
            var flap = 0.5f + 0.5f * Mathf.Sin(Time.time * particleWingFlapSpeed * flapSpeedScale + index * 0.83f);
            var wingColor = Color.Lerp(particle.BodyColor, Color.white, 0.78f);
            wingColor.a = particleWingAlpha * Mathf.Lerp(0.72f, 1f, flap);
            particle.FarWingRenderer.color = wingColor;
            particle.NearWingRenderer.color = wingColor;

            RenderWing(
                particle.FarWingRenderer,
                particleWingFarBackOffsetRatio,
                particleWingFarUpOffsetRatio,
                particleWingFarBaseAngleDegrees,
                flap,
                flapLengthScale);

            RenderWing(
                particle.NearWingRenderer,
                particleWingNearBackOffsetRatio,
                particleWingNearUpOffsetRatio,
                particleWingNearBaseAngleDegrees,
                flap,
                flapLengthScale);
        }

        // 1枚の羽を「背中側+上寄り」に配置し、基準角を中心にflapで開閉させる。
        // Figureのローカル座標系(+x=向いている方向、+y=上)を前提に、-x側(背中)へ置く。
        void RenderWing(SpriteRenderer wing, float backOffsetRatio, float upOffsetRatio, float baseAngleDegrees, float flap, float lengthScale)
        {
            var backOffset = particleSize * backOffsetRatio;
            var upOffset = particleSize * upOffsetRatio;
            var wingWidth = particleSize * particleWingWidthRatio * Mathf.Lerp(0.8f, 1.08f, flap);
            var wingLength = particleSize * particleWingLengthRatio * lengthScale * Mathf.Lerp(1.08f, 0.86f, flap);
            var flapAngle = baseAngleDegrees + Mathf.Lerp(-particleWingFlapAngleDegrees, particleWingFlapAngleDegrees, flap);

            wing.transform.localPosition = new Vector3(-backOffset, upOffset, 0f);
            wing.transform.localScale = new Vector3(wingWidth, wingLength, 1f);
            wing.transform.localRotation = Quaternion.Euler(0f, 0f, flapAngle);
        }

        // 脚2本。付け根(股関節)を支点にlocalRotationだけで振らせる(Capsuleのpivotが上端のため)。
        void RenderParticleLegs(Particle particle, int index, ParticlePose pose)
        {
            if (particle.LegBackRenderer == null || particle.LegFrontRenderer == null)
            {
                return;
            }

            particle.LegBackRenderer.enabled = showParticleLegs;
            particle.LegFrontRenderer.enabled = showParticleLegs;
            if (!showParticleLegs)
            {
                return;
            }
            ...
            {
                case ParticlePoseKind.Climbing:
                    // よじ登り感を出すため、通常より速く・大きく交互に動かす。
                    swing = Mathf.Sin(Time.time * 7f + index * 0.9f) * particleLegClimbSwingDegrees;
                    break;
                case ParticlePoseKind.BloomAttached:
                    // 休憩中: ほぼ静止し、わずかに開く程度。
                    swing = Mathf.Sin(Time.time * 1.2f + index * 0.9f) * 6f;
                    break;
                case ParticlePoseKind.Bursting:
                    // はじけて自由飛散中: 高速にバタつかせる。
                    swing = Mathf.Sin(Time.time * 20f + index * 0.9f) * particleLegBurstSwingDegrees;
                    break;
                default:
                    // 通常飛行: ゆったりゆらゆら。
                    swing = Mathf.Sin(Time.time * 3f + index * 0.9f) * particleLegSwingDegrees;
                    break;
            }

            particle.LegBackRenderer.transform.localRotation = Quaternion.Euler(0f, 0f, swing);
            particle.LegFrontRenderer.transform.localRotation = Quaternion.Euler(0f, 0f, -swing);
        }
        ...
        sealed class Particle
        {
            ...
            // Step P-0: 向き反転(localScale.x=±1)と傾き(localRotation)だけを担当する中間ノード。
            // Root自体は位置専用で常にrotation=identityに保つ(Step P-1)。
            public Transform FigureRoot;

            public SpriteRenderer GlowRenderer;
            public SpriteRenderer FarWingRenderer;
            public SpriteRenderer NearWingRenderer;
            public SpriteRenderer BodyRenderer;
            public SpriteRenderer HeadRenderer;
            public SpriteRenderer LegBackRenderer;
            public SpriteRenderer LegFrontRenderer;

            // Step P-1: 向き(+1右/-1左)と傾き(度)。どちらも毎フレーム平滑更新するため
            // Particleごとにフィールドとして持つ(Domain状態ではなく純粋にView側の見た目状態)。
