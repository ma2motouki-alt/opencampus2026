...
class NearestObjectTracker:
    ...
    def assign_ids(self, detections: list[dict], now: float) -> list[dict]:
        ...
        for detection in detections:
            ...
            self.tracks[match_id] = Track(match_id, float(detection_with_id["x"]), float(detection_with_id["y"]), now)
    ...
    def _find_match(self, detection: dict, candidate_ids: set[int]) -> int | None:
        ...
        for object_id in candidate_ids:
            ...
            distance = math.hypot(float(detection["x"]) - track.x, float(detection["y"]) - track.y)
    ...
    def _drop_stale(self, now: float) -> None:
        for object_id in [object_id for object_id, track in self.tracks.items() if now - track.last_seen > self.ttl_seconds]:
